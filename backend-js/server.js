...
server.get('/latest-blogs', (req, res, next)=> {
	let maxLimit = 5
...
server.post('/search-blogs', (req, res, next)=> {
	let {tag} = req.body
	let findQuery = {tags: tag, draft: false}
	let maxLimit = 5;

	Blog.find(findQuery).populate('author', 'personal_info.profile_img personal_info.username personal_info.fullname -_id').sort({'publishedAt': -1}).select('blog_id title des banner activity tags publishedAt -_id').limit(maxLimit).then(blogs => {
		return res.status(200).json({blogs})
	}).catch(err => {
		return res.status(500).json({error: err.message})
	})
